...
- **对象浏览器**：顶栏 🗃 对象（或侧边栏树里双击表名）→ 打开"数据 / 结构 / DDL"标签页；数据页支持分页、点列头排序、WHERE 过滤、复制为 TSV（可粘贴 Excel）
- **SQL 控制台**：底部 🗄 SQL。单击连接查看数据库信息面板（版本/状态/字符集/会话/存储，Oracle·MySQL·OB 各自适配）；▶/双击新建查询会话窗口（CodeMirror 高亮，Ctrl+Enter 执行，选中可单独执行）；Oracle 信息面板含**活跃会话**交互表——默认 `gv$session where wait_class<>'Idle' and username is not null order by last_call_et` 的常用排障列，列集按当前库 GV$SESSION 真实结构动态获取、可勾选、可刷新
...
- 数据文件位置：`%APPDATA%/sqlpilot/`（config.json、secrets.json、audit.log、sessions.json；旧版 DBAgent 数据首次启动自动迁移）
  ...
  - 对话历史持久化在 sessions.json，重启自动恢复，"清空对话"即删除
